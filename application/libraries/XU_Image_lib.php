...
class CI_Image_lib {
	...
	function initialize($props = array())
	{
		...
		{
			$this->set_error(lang('You must specify a source image in your preferences.'));
			return FALSE;	
		}
		...
		{
			$this->set_error(lang('Your server must support the GD image library in order to determine the image properties.'));
			return FALSE;
		}
	...
	function rotate()
	{
		...
		{
			$this->set_error(lang('An angle of rotation is required to rotate the image.'));
			return FALSE;	
		}
	...
	function image_process_imagemagick($action = 'resize')
	{
		...
		{
			$this->set_error(lang('The path to your image library is not correct.  Please set the correct path in your image preferences.'));
			return FALSE;
		}
		...
		if ($retval > 0)
		{
			$this->set_error(lang('Image processing failed.  Please verify that your server supports the chosen protocol and that the path to your image library is correct.'));
			return FALSE;
		}
	...
	function overlay_watermark()
	{
		...
		{
			$this->set_error(lang('The GD image library is required for this feature.'));
			return FALSE;
		}
	...
	function text_watermark()
	{
		...
		{
			$this->set_error(lang('Unable to find a font to use.'));
			return FALSE;
		}
	...
	function image_create_gd($path = '', $image_type = '')
	{
		...
		switch ($image_type)
		{
			case	 1 :
						...
						{
							$this->set_error(array(lang('Your server does not support the GD function required to process this type of image.'), lang('GIF images are often not supported due to licensing restrictions.  You may have to use JPG or PNG images instead.')));
							return FALSE;
						}
			...
			case 2 :
						...
						{
							$this->set_error(array(lang('Your server does not support the GD function required to process this type of image.'), lang('JPG images are not supported.')));
							return FALSE;
						}
			...
			case 3 :
						...
						{
							$this->set_error(array(lang('Your server does not support the GD function required to process this type of image.'), lang('PNG images are not supported.')));
							return FALSE;
						}
		...
		$this->set_error(array(lang('Your server does not support the GD function required to process this type of image.')));
		return FALSE;
	}
	...
	function image_save_gd($resource)
	{
		switch ($this->image_type)
		{
			case 1 :
						...
						{
							$this->set_error(lang('Unable to save the image. Please make sure the image and file directory are writable.'));
							return FALSE;
						}
				break;
			case 2	:
	...
	function get_image_properties($path = '', $return = FALSE)
	{
		...
		if ( ! file_exists($path))
		{
			$this->set_error(lang('The path to the image is not correct.'));
			return FALSE;
		}
